...
- CLI `--font-size` (`1`–`128`) and `--theme` (`dark` | `light`) on the scenario path and `svg` subcommand.
...
- `svg` subcommand to convert an existing asciicast v2 file.
...
- `[output]` shares a stem; only the extension differs (`.cast` / `.svg`).
- `--font-size` overrides `render.font-size` for the written cast header and any SVG in the same run. Duplicate option is an error.
- `--theme` selects a built-in preset and overrides `render.theme.preset`; YAML `fg` / `bg` / `palette` still merge on top. The cast header stores resolved hex colors only. Duplicate option is an error.
...
- The input cast is read-only; only `.svg` is written.
- `--font-size` and `--theme` override cast-header values for rendering only (precedence: CLI > cast header > defaults).
- Accepts asciicast v2 with required header `width` and `height` (`1`–`512` each).
...
Every cast file includes render metadata in the header, whether or not `--format svg` is used.
