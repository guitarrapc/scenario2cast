...
- **Child stdin must not be closed before launch completes.** Closing the input pipe too early can yield `STATUS_CONTROL_C_EXIT` (0xC000013A) rather than normal command output.
- **Stdin EOF must be signaled for one-shot input.** After writing bytes, call `CloseInput()` before waiting. Windows closes the ConPTY input pipe; Unix sends EOT (`0x04`) because PTY master fds cannot be half-closed with `shutdown(SHUT_WR)`. Do not close the input pipe via `FileStream` disposal on the write handle — use `WriteFile` and an explicit `CloseInput()`.
