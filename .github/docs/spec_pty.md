...
- **Stdin EOF must be signaled for one-shot input.** After writing bytes, call `CloseInput()` before waiting; otherwise children that block on stdin EOF (`cat`, `ReadToEnd`, etc.) hang until killed. Do not close the input pipe via `FileStream` disposal on the write handle — use `WriteFile` and an explicit `CloseInput()`.
