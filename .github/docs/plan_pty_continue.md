...
| G5 | PTY step でも通常 step と同じプロンプト + タイピング + Enter をデフォルトで emit できる |

## Non-Goals

| # | Non-Goal | 理由 |
|---|---|---|
| N1 | PTY 実行中の意図的な `clear` / `cls` / TUI の画面操作を抑制する | コマンド本体の挙動であり、初期化フィルタでは判別不能。alternate screen 終了直後の main screen cleanup は例外的に対象 |
| N2 | シェル起動方式の変更（`-lc` → `-c` など）をデフォルトで行う | 副作用（プロファイル未読込、環境変数差分）が大きい |
| N3 | 外部 `.cast` の `svg` 変換時にフィルタを適用する | シナリオ録画パスのみが対象 |
| N4 | カーソル位置を「前 step の末尾」へ明示的に再配置する | 初期化シーケンス除去でカーソルは自然に前位置を維持する想定。不足があれば follow-up |
...
| `pty: true` | PTY 出力前にプロンプト、コマンドのタイピング、Enter を記録 |
...
| `run-highlight` | PTY step の typed command にも適用される |
...
| PTY step | プロンプト、typed command、Enter が PTY 出力前に emit される |
...
| Q2 | `pty-continue: true` 時もタイピング演出を付けるか？ | PTY step のデフォルトとして実装済み |
...
- PTY 実行そのものはシェルコマンドとして起動し、タイピングは cast 上の演出だけに限定すると、対話 stdin の扱いを増やさずに通常 step と同じ見た目へ寄せられる。
