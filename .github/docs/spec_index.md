...
| [spec_pty.md](spec_pty.md) | PTY recording behavior when `pty: true` (keys in [spec_scenario.md](spec_scenario.md)) |
...
| Continue the previous screen after a PTY command or alternate-screen TUI | [spec_scenario.md](spec_scenario.md) → `pty-continue`; behavior in [spec_pty.md](spec_pty.md) |
...
spec_scenario.md          ← YAML input (start here for file format)
    ...
    ├── spec_pty.md           pty:true recording behavior
...
- **Step exit codes** live in [spec_scenario.md](spec_scenario.md) → Step exit codes; warning text in [spec_cli.md](spec_cli.md).
- **PTY recording** lives in [spec_pty.md](spec_pty.md); keys live in [spec_scenario.md](spec_scenario.md).
